...
@pytest.fixture
def post_cleanup(logged_in_bot):
    """A fixture that runs after a test to clean up all posts by the user."""
    yield
    # Teardown: delete all posts made by the bot during the test
    util.del_all_post(logged_in_bot)


def test_post_and_comment(logged_in_bot: PyPtt.API, post_cleanup):
    """Tests posting, commenting, and verifying the post content."""
    ptt_bot = logged_in_bot
    content = '''
